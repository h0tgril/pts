...
class Server:
  ...
  def broadcast_event(self, event, pos):
    self.event_centers.append((event, pos, 0))

  def expand_events(self):
    def circle(center, radius):
      # TODO TODO
      return []
    for event, center, radius in self.event_centers:
      old = circle(center, radius)
  ...
  def enqueue_move(self, x, y, dx, dy, detour=False):
    ...
    if occupied:
      if occupied.player.index != unit.player.index:
        ...
        self.broadcast_event(Event(self.tick_no, unit,
          old_pos=(new_x, new_y), new_pos=None), (x, y))
...
#                  "move": (move_x, move_y)})
        hue = 360 * obj.player.index / len(server.players) + (obj.skin[0] / 12)
        hue = int(hue) % 360
        color = pygame.Color(0)
